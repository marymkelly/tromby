...
window.onload = function () {
	...
	if(document.querySelector('.wrap')){ //if about page
		return
	}
	...
	document.querySelector('#controls').addEventListener('click', () => {
		...
		let directionStatus = document.getElementById('ctl-status');

		if (containerRow.classList.contains('hide')) {
			containerRow.classList.remove('hide');
			document.querySelector('#controls').classList.add('open');
			directionStatus.innerHTML = 'Close';
			directions = true;
		} else {
			containerRow.classList.add('hide');
			document.querySelector('#controls').classList.remove('open');
			directionStatus.innerHTML = 'Open';
			directions = false;
		}
	})
...
function handler(event) {
	//console.log(event);
	if (directions) {
		if (event.getModifierState("Control") || //ignore some modifier keys
			event.getModifierState("Alt") ||
			event.getModifierState("Meta")) {
			return;
		}
		//prevent simulator control in backround
